...
async function main() {
    try {
        ...
        const [userMasterRaw, unitMasterRaw, unitMgmtRaw, mealCountRaw] = await Promise.all([
            readSheet(token, '利用者マスタ!A:M'),
            readSheet(token, 'ユニットマスタ!A:C'),
            readSheet(token, 'ユニット管理!A:E'),
            readSheet(token, '食数計算!A:H')
        ]);

        const users = parseUserMaster(userMasterRaw);
        const units = parseUnitMaster(unitMasterRaw);

        console.log('Parsing UnitManagement...');
        const mgmt = parseUnitManagement(unitMgmtRaw);

        console.log('Parsing MealCount...');
        const meals = parseMealCount(mealCountRaw);

        console.log(`Loaded ${users.length} users, ${units.length} units, ${mgmt.length} management records, ${meals.length} meal records.`);
